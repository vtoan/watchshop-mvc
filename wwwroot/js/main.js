...
function updateViewItemCart() {
	$('#cart-items').text(order.items.reduce((t, item) => (t = t + item.quantity), 0));
}
...
function onAddCart() {
	$('.add-cart').on('click', function () {
		addItemToCart($(this).parents('.product').data('itemid'));
		updateViewItemCart();
		initPopup('Thêm vào giỏ hàng thành công', 'Xem Giỏ hàng', 'Không', '/Cart');
	});
}
/*=======?ui-elemetn======= */
function initPopup(
...
function renderProducts(items, target) {
	setTimeout(function () {
		...
		if (!items || items.length == 0)
			container.removeClass('row').append(`<p class="text-center mt-5 text-4">Không có sản phẩm phù hợp</p>`)
		else {
...
function crtProductElm(p) {
	return `<div class="col-6 col-lg-3">
                    <div class="product ${ p.discount == 0 ? '': 'sale'}" data-itemid=${p.id}>
                        <div class="product-img img-content">
                            <div class="img">
                                 <img src="/img/${p.image}" loading="lazy">
                            </div>
                            <a class="add-cart text-center d-none d-lg-block" href="#!">Add to Cart</a>
                        </div>
                        <a href="/Product/Detail?id=${p.id}" class="product-content">
                            <h4 class="pb-2 normal">${p.name} - ${p.wireID}</h4>
                            <div class="text-center">
                                <p class="text-4 red bold d-block d-lg-inline-block m-0">${cvtIntToMoney(p.price - p.discount)} đ</p>
